...
class RHPE(handdataset.HandDataset):
    def __init__(
        self,
        root,
        cache_root=None,
        ...
        **kwargs,
        ...
        self.split_folder = "RHPE_TRAIN" if train else "RHPE_VAL"
        self.split_folder = os.path.join(root, self.split_folder)
        self.roi_folder = os.path.join(root, "RHPE_ANNOTATIONS/ANATOMICAL_ROIS")
        self.json_filename = (
            "RHPE_Anatomical_ROIs_Training.json"
            if train
            else "RHPE_Anatomical_ROIs_Validation.json"
        )
    ...
    def _load_annotations(self, split):
        ...
        for each in annots["annotations"]:
