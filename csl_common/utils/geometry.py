...
def bboxRelOverlap(bbox1, bbox2):
    ...
    intersect_x_edge = min(bbox1[0] + bbox1[2], bbox2[0] + bbox2[2]) - max(
        bbox1[0] - bbox1[2], bbox2[0] - bbox2[2]
    )
    intersect_y_edge = min(bbox1[1] + bbox1[3], bbox2[1] + bbox2[3]) - max(
...
def bboxIntersection(bbox1, bbox2, typeBB):
    if typeBB == 1:
        ...
        bb1 = np.array(bbox1)
        bb2 = np.array(bbox2)
    bbIntersect = np.hstack(
        (np.maximum(bb1[:2], bb2[:2]), np.minimum(bb1[2:4], bb2[2:4]))
    )
    if typeBB == 1:
