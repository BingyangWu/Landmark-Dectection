...
def make_grid(
    data, padsize=2, padval=255, nCols=10, dsize=None, fx=None, fy=None, normalize=False
):
    ...
    padding = ((0, r * c - data.shape[0]), (0, padsize), (0, padsize)) + ((0, 0),) * (
        data.ndim - 3
    )
    data = np.pad(data, padding, mode="constant", constant_values=(padval, padval))

    # tile the filters into an image
    data = data.reshape((r, c) + data.shape[1:]).transpose(
    ...
    if dsize is not None or fx is not None or fy is not None:
        data = cv2.resize(
            data, dsize=dsize, fx=fx, fy=fy, interpolation=cv2.INTER_LANCZOS4
        )
...
def to_disp_image(img, denorm=False, output_dtype=np.uint8):
    ...
    if denorm:
        img = normalizer.detransform(img)
    ...
    if img.max() > 32.00:
        if isinstance(img, np.ndarray):
            ...
            raise ValueError(
                "Image data in wrong value range (min/max={:.2f}/{:.2f}).".format(
                    img.min(), img.max()
                )
            )
    """
...
def reconstruct_image(img, output_dtype=np.uint8):
    if not isinstance(img, np.ndarray):
        img = img.detach().cpu().numpy()
    img = img.astype(np.float32).copy()
    if img.shape[0] == 3 or img.shape[0] == 1:
    ...
    if img.min() < -1 or img.max() > 2:
        print(
            "warning: image may not be valid (min/max={.2f}/{.2f})".format(
                img.min(), img.max()
            )
        )
    img = np.clip(img, a_min=0, a_max=1)
...
def reconstruct_images(imgs):
    return [reconstruct_image(i) for i in imgs]
...
def add_frames_to_images(images, labels, label_colors, gt_labels=None):
    ...
    for idx, (disp, label) in enumerate(zip(new_images, labels)):
        ...
        if gt_labels is not None:
            ...
            cv2.circle(
                disp, (disp.shape[1] - 2 * radius, 2 * radius), radius, color, -1
            )
    return new_images
...
def get_pos_in_image(loc, text_size, image_shape):
    ...
    #     pos = (mid_offset, image_shape[0]-bottom_offset-line_height)
        ...
        pos = (
            image_shape[1] - right_offset,
            image_shape[0] - bottom_offset - line_height,
        )
    elif loc == "br-2":
        pos = (
            image_shape[1] - right_offset,
            image_shape[0] - bottom_offset - 2 * line_height,
        )
    elif loc == "bm":
        pos = (image_shape[1] - right_offset * 2, image_shape[0] - bottom_offset)
    elif loc == "bm-1":
        pos = (
            image_shape[1] - right_offset * 2,
            image_shape[0] - bottom_offset - line_height,
        )
    elif loc == "bm-2":
        pos = (
...
def add_id_to_images(
    images, ids, gt_ids=None, loc="tl", color=(1, 1, 1), size=0.7, thickness=1
):
    new_images = to_disp_images(images)
...
def add_landmarks_to_images(
    ...
    def draw_wireframe_skeleton(img, lms, skeleton):
        pts = lms.reshape((-1, 2)).astype(np.int32)
        for each in skeleton:
            p1 = tuple(pts[each[0] - 1].astype(int))
            p2 = tuple(pts[each[1] - 1].astype(int))
            cv2.line(img, p1, p2, color=color, lineType=cv2.LINE_AA)
    ...
    new_images = to_disp_images(images, denorm=denorm)
    ...
    if gt_landmarks is not None and draw_gt_offsets:
        for img_id in range(len(new_images)):
            ...
            draw_offset_lines(
                new_images[img_id], landmarks[img_id], gt_landmarks[img_id], dists
            )

    for img_id, (disp, lm) in enumerate(zip(new_images, landmarks)):
        if draw_dots:
            for lm_id in range(0, len(lm)):
                if landmarks_to_draw is None or lm_id in landmarks_to_draw:
                    lm_color = color
                    if lm_color is None:
                        if lm_errs is not None:
                            lm_color = color_map(
                                lm_errs[img_id, lm_id],
                                cmap=plt.cm.jet,
                                vmin=0,
                                vmax=1.0,
                            )
                    ...
                    cv2.circle(
                        disp,
                        tuple(lm[lm_id].astype(int).clip(0, disp.shape[0] - 1)),
                        radius=radius,
                        color=lm_color,
                        thickness=-1,
                        lineType=cv2.LINE_AA,
                    ...
                    if lm_confs is not None:
                        ...
                        try:
                            conf_radius = max(
                                2, int((1 - lm_confs[img_id, lm_id]) * max_radius)
                            )
                        except ValueError:
                            conf_radius = 2
        ...
        if draw_wireframe:
            draw_wireframe_skeleton(disp, lm, skeleton)
    return new_images
...
def draw_z(z_vecs):
    ...
    for lvl, _ft in enumerate(to_numpy(z_vecs)):
        ...
        canvas[: int(fy * len(_ft)), :] = color_map(
            cv2.resize(
                _ft.reshape(-1, 1),
                dsize=(width, int(fy * len(_ft))),
    ...
    return make_grid(z_zoomed, nCols=len(z_vecs), padsize=1, padval=0).transpose(
        (1, 0, 2)
    )
...
def overlay_heatmap(img, hm, heatmap_opacity=0.45):
    ...
    if len(hm.shape) > 2:
        ...
        if hm_colored.shape != img.shape:
            hm_colored = cv2.resize(
                hm_colored, (img.shape[1], img.shape[0]), interpolation=cv2.INTER_CUBIC
            )
...
def draw_z_vecs(levels_z, ncols, size, vmin=-1, vmax=1, vertical=False):
    ...
    def draw_z(z):
        ...
        for lvl, ft in enumerate(z):
            ...
            canvas = color_map(
                cv2.resize(
                    _ft_reshaped, dsize=(width, height), interpolation=cv2.INTER_NEAREST
                ),
                vmin=vmin,
                vmax=vmax,
            )
    ...
    z_grid_per_sample = [
        make_grid(all_vis_sample, nCols=len(levels_z))
        for all_vis_sample in zip(*z_vis_list_per_level)
    ]
