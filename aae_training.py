...
def calc_ssim(X, X_recon):
    ...
    for i in range(len(X)):
        ...
        ssim[i] = compare_ssim(
            input_images[i], recon_images[i], data_range=data_range, multichannel=True
        )
    return ssim
...
class AAETraining(object):
    def __init__(
        ...
        if snapshot is not None:
            log.info(
                "Resuming session {} from snapshot {}...".format(
        ...
        self.optimizer_D_z = optim.Adam(
            self.saae.D_z.parameters(), lr=args.lr, betas=betas
        )
        self.optimizer_D = optim.Adam(
            self.saae.D.parameters(), lr=args.lr * 0.5, betas=betas
        )
    ...
    def _load_snapshot(self, snapshot_name, data_dir=None):
        ...
        log.info(
            "Model {} trained for {} iterations ({}).".format(
                snapshot_name, self.total_iter, str_training_time
            )
        )

    def _is_snapshot_iter(self):
        return (self.total_iter + 1) % self.snapshot_interval == 0 and (
            self.total_iter + 1
        ) > 0
    ...
    def _is_printout_iter(self, eval):
        return (self.iter_in_epoch + 1) % self._print_interval(
            eval
        ) == 0 or self.iter_in_epoch + 1 == self.iters_per_epoch
    ...
    def update_gan(
        ...
        if self.iter_in_epoch % self.args.update_D_freq == 0:
            ...
            loss_D = -torch.mean(
                torch.log(err_real + eps) + torch.log(1.0 - err_fake + eps)
            )
            if with_gen_loss:
                err_fake_gen = self.saae.D(X_gen.detach())
                loss_D_gen = -torch.mean(
                    torch.log(err_real + eps) + torch.log(1.0 - err_fake_gen + eps)
                )
                loss_D = loss_D * (1 - w_gen) + loss_D_gen * w_gen
                stats.update(
                    {"loss_D_rec": loss_D.item(), "loss_D_gen": loss_D_gen.item()}
                )
            if train:
                loss_D.backward()
        ...
        if self.iter_in_epoch % self.args.update_E_freq == 0:
            ...
            if with_gen_loss:
                ...
                stats.update(
                    {"loss_G_rec": loss_G_rec.item(), "loss_G_gen": loss_G_gen.item()}
                )
            else:
                loss_G = loss_G_rec
    ...
    def visualize_interpolations(self, Z, nimgs=1, ninterp=8, target_id=-1, wait=10):
        ...
        cv2.imwrite("interpolations.jpg", cv2.cvtColor(disp_rows, cv2.COLOR_RGB2BGR))
        cv2.waitKey(wait)
    ...
    def visualize_random_images(self, nimgs=8, wait=10, z_real=None, real_dist=0.5):
        ...
        cv2.imwrite("random images.jpg", cv2.cvtColor(disp_rows, cv2.COLOR_RGB2BGR))
        cv2.waitKey(wait)
    ...
    def visualize_batch(self, batch, X_recon, ssim_maps, nimgs=8, ds=None, wait=0):
        ...
        if ssim_maps is not None:
            ...
            cv2.imwrite(
                "ssim errors.jpg", cv2.cvtColor(grid_ssim_maps, cv2.COLOR_RGB2BGR)
            )
        ...
        cv2.imwrite(wnd_title + ".jpg", cv2.cvtColor(disp_rows, cv2.COLOR_RGB2BGR))
        cv2.waitKey(wait)
    ...
    def reconstruct_fixed_samples(self):
        ...
        for phase, b in self.fixed_batch.items():
            ...
            img = vis_reconstruction(
                self.saae, b.images, landmarks=b.landmarks, ncols=5, fx=f, fy=f
            )
...
def add_arguments(parser, defaults=None):
    ...
    # model params
    parser.add_argument(
        "--sessionname", default=None, type=str, help="output filename (without ext)"
    )
    parser.add_argument(
        "-r",
        "--resume",
    ...
    parser.add_argument(
        "-i", "--input-size", default=256, type=int, help="CNN input size"
    )
    ...
    parser.add_argument(
        "--train-encoder",
        type=bool_str,
        default=defaults.get("train_encoder", True),
        help="include encoder update in training ",
    )
    parser.add_argument(
        "--train-decoder",
        type=bool_str,
        default=defaults.get("train_decoder", True),
    ...
    parser.add_argument(
        "-b",
        "--batchsize",
        default=defaults.get("batchsize", 50),
        type=int,
        metavar="N",
        help="batch size",
    )
    ...
    parser.add_argument(
        "--reset", default=False, action="store_true", help="reset the discriminator"
    )
    parser.add_argument(
        "--lr", default=0.00002, type=float, help="learning rate for autoencoder"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="save snapshot every N epochs",
    )
    parser.add_argument(
        "--print-freq",
        "-p",
    ...
    parser.add_argument(
        "--eval-freq", default=10, type=int, metavar="N", help="evaluate every N steps"
    )
    parser.add_argument(
        "--batchsize-eval",
        default=20,
    ...
    # data
    parser.add_argument(
        "--use-cache", type=bool_str, default=True, help="use cached crops"
    )
    parser.add_argument(
        "--train-count",
        default=None,
    ...
    parser.add_argument(
        "--st", default=None, type=int, help="skip first n training images"
    )
    parser.add_argument(
        "--val-count", default=None, type=int, help="number of test images"
    )
    parser.add_argument(
        "--daug", type=int, default=0, help="level of data augmentation for training"
    )
    parser.add_argument(
        "--align",
        type=bool_str,
        default=False,
        help="rotate crop so eyes are horizontal",
    )
    parser.add_argument(
        "--occ", type=bool_str, default=False, help="add occlusions to target images"
    )
    parser.add_argument(
        "--crop-source",
        type=str,
        default="bb_ground_truth",
    ...
    # visualization
    parser.add_argument(
        "--show", type=bool_str, default=True, help="visualize training"
    )
