...
class FaceCrop:
    def __init__(
        ...
        if crop_by_eye_mouth_dist and crop_by_height:
            raise ValueError(
                "'crop_by_eye_mouth_dist' and 'crop_by_height' cannot both be true. "
    ...
    def __get_eye_coordinates(self, lms):
        ...
        eye_l, eye_r = lms[id_eye_l].mean(axis=0), lms[id_eye_r].mean(
            axis=0
        )  # eye centers
        if eye_r[0] < eye_l[0]:
    ...
    def calculate_crop_parameters(self, img, lms_orig):
        ...
        self.face_center, self.face_scale = self.__get_face_center(
            lms, return_scale=True
        )
        ...
        if self.align_face_orientation:
            ...
            chin = self.__get_chin_coordinates(lms)
            nose_upper = lms[51]
            self.M = self.__calc_rotation_matrix(
                self.face_center, eye_l, eye_r, nose_upper, chin
            )
        ...
        if self.crop_by_eye_mouth_dist:
            ...
            # enlarge a little
            min_row, max_row = int(t - self.crop_move_top_factor * h), int(
                b + self.crop_move_bottom_factor * h
            )
    ...
    def apply_to_image(self, img=None, with_hist_norm=False, border_mode="black"):
        ...
        padding = [(dilate[1], dilate[1]), (dilate[0], dilate[0]), (0, 0)]
        try:
        ...
        if self.align_face_orientation and self.lms is not None:
            ...
            M = cv2.getRotationMatrix2D(
                tuple(face_center + (w, h)), self.angle_x_deg, 1.0
            )
        ...
        try:
            resized_crop = cv2.resize(
                crop, self.output_size, interpolation=cv2.INTER_CUBIC
            )
        except cv2.error:
    ...
    def apply_to_landmarks(self, lms_orig, pose=None):
        ...
        if self.align_face_orientation and self.lms is not None:
            ...
            M = cv2.getRotationMatrix2D(
                tuple(self.face_center_orig), self.angle_x_deg, 1.0
            )
