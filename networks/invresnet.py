...
class LandmarkHead(nn.Module):
    def __init__(
        ...
        self.lin = nn.ConvTranspose2d(
            64, output_channels, kernel_size=4, stride=2, padding=1, bias=False
        )
...
class InvResNet(nn.Module):
    def __init__(
        ...
        inplanes_after_layer1 = (
            self.inplanes
        )  # self.inplanes gets changed in _make_layers
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2)
        inplanes_after_layer2 = (
            self.inplanes
        )  # self.inplanes gets changed in _make_layers
...
class InvResNetParallel(nn.DataParallel):
