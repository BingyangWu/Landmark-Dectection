...
def gaussian(x, mean, sigma):
    return (
        1 / (sigma * np.sqrt(2 * np.pi)) * np.exp(-((x - mean) ** 2) / (2 * sigma ** 2))
    )
...
def _fill_heatmap_layer(dst, lms, lm_id, lm_heatmap_window, wnd_size):
    posx, posy = min(lms[lm_id, 0], lmcfg.HEATMAP_SIZE - 1), min(
        lms[lm_id, 1], lmcfg.HEATMAP_SIZE - 1
    )
    ...
    try:
        ...
        weight = 1.0
        dst[
            cn, max(0, t) : min(img_size, b), max(0, l) : min(img_size, r)
        ] = np.maximum(
...
def decode_heatmaps(hms):
    ...
    for lm in range(0, 68):
        ...
        lms[lm, :] = np.unravel_index(np.argmax(heatmap, axis=None), heatmap.shape)[
            ::-1
        ]
...
def calc_landmark_nme_per_img(
    gt_lms, pred_lms, ocular_norm="pupil", landmarks_to_eval=None, image_size=None
):
...
def calc_landmark_nme(gt_lms, pred_lms, ocular_norm="pupil", image_size=None):
    ...
    if gt.shape[1] == 5:  # VGGFace2
        ...
        assert image_size is not None
        ocular_dists = np.ones(gt.shape[0], dtype=np.float32) * image_size
    norm_dists = np.sqrt(np.sum((gt - pred) ** 2, axis=2)) / ocular_dists.reshape(
        len(gt), 1
    )
...
def calc_landmark_ncc(X, X_recon, lms):
    ...
    for i in range(nimgs):
        for lid in range(nlms):
            ...
            ncc = ((wnd1 - wnd1.mean()) * (wnd2 - wnd2.mean())).mean() / (
                wnd1.std() * wnd2.std()
            )
            nccs[i, lid] = ncc
...
def convert_landmarks(lms, code):
    ...
    if len(lms.shape) == 3:
        # new_lms = []
        new_lms = torch.zeros(
            (len(lms), 5, 2), requires_grad=True, dtype=torch.float32
        ).cuda()
