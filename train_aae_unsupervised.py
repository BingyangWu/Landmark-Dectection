...
class AAEUnsupervisedTraining(AAETraining):
    ...
    def _get_network(self, pretrained):
        return aae.AAE(self.args.input_size, pretrained_encoder=pretrained).cuda()
    ...
    def _print_iter_stats(self, stats):
        ...
        str_stats[0] += [
            ...
            "{t_data:.2f}/{t_proc:.2f}/{t:.2f}s ssim_cost:{t_ssim:.2f} ({total_iter:06d} {epoch_time} expected_time_left:{expected_time_left})"
        ][0]
        ...
        log.info(
            str_stats[0].format(
                ...
                t_ssim=means["time_ssim"],
                total_iter=self.total_iter + 1,
                epoch_time=str(datetime.timedelta(seconds=duration)),
                expected_time_left=str(
                    datetime.timedelta(
                        seconds=int(
                            duration / (current["iter"] + 1) * self.iters_per_epoch
                            - duration
                        )
                    )
                ),
    ...
    def eval_epoch(self):
        ...
        self._print_epoch_summary(self.epoch_stats, eval=True)
    ...
    def train(self, num_epochs):
        ...
        while num_epochs is None or self.epoch < num_epochs:
            ...
            if (
                self.epoch + 1
            ) % self.snapshot_interval == 0 or self.epoch == num_epochs - 1:
    ...
    def _run_epoch(self, dataset, eval=False):
        ...
        if eval:
            dataloader = td.DataLoader(
                dataset, batch_size=batchsize, num_workers=self.workers
            )
        else:
    ...
    def _run_batch(self, data, eval=False, ds=None):
        ...
        with torch.set_grad_enabled(self.args.train_encoder):
            ...
            ###########################
            if (
                (not eval or self._is_printout_iter(eval))
                and self.args.with_zgan
                and self.args.train_encoder
            ):
                if WITH_LOSS_ZREG:
        ...
        with torch.set_grad_enabled(self.args.train_decoder):
            ...
            if self.args.with_ssim_loss or eval:
                store_cs_maps = (
                    self._is_printout_iter(eval) or eval
                )  # get error maps for visualization
                loss_ssim, cs_error_maps = aae_training.loss_struct(
                    X_target, X_recon, self.ssim, calc_error_maps=store_cs_maps
                )
...
def run(args):
    ...
    phase_cfg = {
        TRAIN: {"dsnames": args.dataset_train, "count": args.train_count},
        VAL: {"dsnames": args.dataset_val, "count": args.val_count},
    }
    datasets = {}
    for phase in args.phases:
        ...
        if is_single_dataset:
            ...
            datasets[phase] = multi.ConcatFaceDataset(
                datasets_for_phase, max_samples=args.train_count_multi
            )

        print(datasets[phase])

    fntr = AAEUnsupervisedTraining(
        datasets,
        args,
...
if __name__ == "__main__":
    ...
    # Autoencoder losses
    parser.add_argument(
        "--with-gan", type=bool_str, default=False, help="use GAN image loss(es)"
    )
    parser.add_argument(
        "--with-gen-loss",
        type=bool_str,
        default=False,
        help="with generative image loss",
    )
    parser.add_argument(
        "--with-ssim-loss", type=bool_str, default=False, help="with structural loss"
    )
    parser.add_argument(
        "--with-zgan", type=bool_str, default=True, help="with hidden vector loss"
    )
    parser.add_argument(
        "--w-gen", default=0.25, type=float, help="weight of generative image loss"
    )
    parser.add_argument("--w-rec", default=1.0, type=float, help="weight of pixel loss")
    parser.add_argument(
        "--w-ssim", default=60.0, type=float, help="weight of structural image loss"
    )
    parser.add_argument(
        "--update-D-freq",
        default=2,
    ...
    parser.add_argument(
        "--update-E-freq", default=1, type=int, help="update the encoder every N steps"
    )
